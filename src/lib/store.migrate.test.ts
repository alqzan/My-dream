...
describe("لا ترمي أبداً — رميةٌ واحدة تعني إقلاعاً على تطبيقٍ فارغ", () => {
  ...
  // كُشفت هذه الحالةُ عند كتابة الملفّ (٠٫١٫٤٢٤) ووُثّقت، ثمّ حُرست (٠٫١٫٤٢٦):
  // الحراسةُ داخل السلسلة كلُّها `?? []` وهي تمسك الغياب لا **النوعَ الخطأ**،
  // فكتلةٌ فيها `transactions: "نصّ"` كانت ترمي، و`persist` يبتلع الرمية،
  // فيُقلع المالك على تطبيقٍ فارغ والكتلةُ المشوّهة باقيةٌ تُعيد الكرّة كلَّ مرّة.
  it("نوعٌ خطأ يُسوَّى إلى قائمةٍ فارغة ولا يُسقط بقيّةَ الحالة", () => {
    const junk = {
      transactions: "لا قائمة", budgets: 42, categories: null,
      reserves: { nope: true }, habits: undefined, themePalette: 99,
      journalEntries: [{ id: "j1", date: "2026-01-01", content: "نجا" }],
    };
    for (const v of [1, 3, 8, 16, 17, 18]) {
      expect(() => migratePersisted(structuredClone(junk), v), `v${v}`).not.toThrow();
    }
    // والأهمّ: ما كان سليماً في الكتلة يعبر ولا يسقط مع المشوّه.
    const out = migratePersisted(structuredClone(junk), 17);
    expect(rows(out, "journalEntries")).toEqual([{ id: "j1", date: "2026-01-01", content: "نجا" }]);
    expect(rows(out, "transactions")).toEqual([]);
  });
