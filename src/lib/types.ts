...
/** رحلةٌ واحدة على مظروف: نافذةٌ زمنية مغلقةٌ بتاريخين. `id` ليس زينة — هو ما
 *  تتّحد به الرحلاتُ في الدمج، فرحلتان بدأتا على جهازين تبقيان اثنتين ولا
 *  تُلغي إحداهما الأخرى. */
export interface Trip {
  id: string;
  startedAt: string;  // YYYY-MM-DD
  endedAt?: string;   // YYYY-MM-DD — غيابُه يعني «جارية»
}
...
// ===================== خطة تمويل المظروف =====================
// المظروف هو الوعاء الذي **يمتدّ على عدة دورات**، والبدل اليومي دورةٌ واحدة.
// فالصدمة الكبيرة (رحلة، رسوم) لا تُوزَّع على السجل — المال خرج في يومه ويبقى
// مسجّلاً فيه — بل يُوزَّع **تمويلُها**: مبلغٌ ثابت ينتقل إلى المظروف كل دورة
// راتب. اتجاهان للشيء نفسه: استعداداً لحدثٍ قادم، أو سداداً لمظروفٍ صار سالباً
// بعد حدثٍ وقع. التطبيق في `confirmSalary`، والحساب في `src/lib/fundPlan.ts`.
export interface FundFunding {
  perCycle: number; // المبلغ المنقول كل دورة راتب
  // **من أين؟** وهذا هو السؤال الصادق: `surplus` من «الفوائض» (مالٌ متراكم من
  // دوراتٍ ماضية — لا يمسّ بدلك اليومي)، و`salary` من راتب الدورة الجديدة —
  // فينزل بدلك اليومي بحصّة اليوم منه (`fundingPerDay`)، وإلّا صرفتَ الريال
  // مرّتين: مرّةً في المظروف ومرّةً في يوميّتك.
  source: "salary" | "surplus";
  // **متى تتوقّف؟** ثلاث حالاتٍ بآليةٍ واحدة:
  //   • غياب الحقل → **مستمرّة بلا نهاية**: الإيجار وكلّ التزامٍ يتكرّر كل دورة.
  //   • "zero"     → **سداد**: تتوقّف حين يصفّر عجزُ المظروف (رحلةٌ وقعت ولم تُموَّل).
  //   • "target"   → **ادخار**: تتوقّف حين يبلغ الرصيدُ هدفَ المظروف (رحلةٌ قادمة).
  // التوقّفُ ذاتيّ فلا يبقى تحويلٌ منسيّ يسحب من راتبك بلا سبب.
  stop?: "zero" | "target";
}
...
export interface ReserveFund {
  ...
  // خطة التمويل الدوري (اختيارية) — انظر `FundFunding` أعلاه.
  funding?: FundFunding;
  // **وضع السفر**: حين يبدأ المالك رحلةً على مظروف حدثٍ مخصّص، يصير كلُّ
  // مصروفٍ يسجّله محسوباً عليها تلقائياً حتى ينهيها — فلا يُسأل عن الوجهة عند
  // كلّ فاتورةٍ وهو في الطريق. مظروفا «عام» و«الفوائض» أوعيةُ احتفاظٍ وليسا
  // رحلات؛ البوابة في `trip.ts` تمنع تشغيل الوضع عليهما. والتاريخان يجعلان
  // السؤال الأخير ممكناً: «كم كلّفتني رحلةُ المدينة، وفي كم يوم؟». الجاريةُ هي
  // التي لا `endedAt` لها، وواحدةٌ في كلّ وقتٍ عبر المظاريف كلِّها
  // (`startTrip` تُنهي ما قبلها).
  //
  // **قائمةٌ لا رحلةً واحدة (٠٫١٫٤٢٥)**: كان الحقلُ `trip` مفرداً، فبدءُ رحلةٍ
  // ثانية على المظروف نفسِه — وهو أطبعُ ما يُفعل بمظروفٍ اسمُه «سفر» — **يكتب
  // فوق الأولى فتختفي بلا استرجاع**، ويقسم `tripSummary` صرفَ المظروف كلَّه على
  // أيّام الثانية فيُعطي رقماً أضعافَ الحقيقة. الآن لكلّ رحلةٍ نافذتُها الزمنية
  // ومعرّفُها، والتقريرُ يقرأ ما وقع **داخل نافذتها** وحدها.
  trips?: Trip[];
...
export interface JournalEntry {
  ...
  // معرّفاتُ الأسطر السريعة المُلحقة بهذه المذكرة (`journalQuickLine.ts`):
  // سجلٌّ لما عُلم كي لا يُسقط الدمجُ سطراً أُضيف على جهازٍ آخر. النصُّ في `content`.
  quickLines?: JournalQuickLine[];
}

export interface JournalQuickLine {
  id: string;
  /** الفقرةُ كما أُلحقت: «١٤:٠٥ — …» */
  text: string;
}
...
/**
 * حضورُ القلب في صلاةٍ بعينها — ثلاثُ درجاتٍ لا أكثر.
 *
 * **لماذا ثلاث؟** السؤال يُطرح خمسَ مرّاتٍ في اليوم بعد كلّ تسجيل، فإن كلّف
 * تفكيراً صار عبئاً يُتخطّى ثمّ يُهجر السجلُّ كلُّه. ثلاثةُ خياراتٍ تُقرأ في
 * لمحةٍ وتُجاب بإصبعٍ واحدة، وهي كافيةٌ لتُظهر فرقاً بين جماعةٍ وفرادى وبين
 * فجرٍ وعصر — وذلك كلُّ ما يُرجى من الرقم.
 *
 * ورقمٌ لا وسم: المتوسّطُ لا يُحسب على النصّ، والدرجاتُ مرتّبةٌ فعلاً (٣ أعلى
 * من ٢ أعلى من ١)، فيصحّ جمعُها ومقارنتُها.
 */
export type KhushuLevel = 1 | 2 | 3;

export const KHUSHU_LEVELS: KhushuLevel[] = [1, 2, 3];

/**
 * **البوّابةُ الوحيدة** لوصف درجة الخشوع. المسمّياتُ مقصودةٌ بلا توبيخ: أدناها
 * «شارد» لا «سيّئ» — لأنّ المالك هو الذي يسجّل على نفسه، وسجلٌّ يعاقب صاحبَه
 * على صدقه يُهجَر أو يُملأ بالكذب، وكلاهما يُفسد الإحصاء.
 */
export const KHUSHU_META: Record<KhushuLevel, { label: string; hint: string; color: string; glyph: string }> = {
  1: { label: "شارد", hint: "كنتُ في وادٍ آخر", color: "#c15a34", glyph: "○" },
  2: { label: "حاضر", hint: "عرفتُ ما أقول", color: "#dc9f3c", glyph: "◐" },
  3: { label: "خاشع", hint: "وجدتُ لها أثرًا", color: "#1f7a6c", glyph: "●" },
};

/** وصفُ درجةٍ قد تأتي من نسخةٍ احتياطية أو جهازٍ أقدم — لا تفهرس الجدول رأساً. */
export function khushuMeta(level: KhushuLevel | number | undefined) {
  return KHUSHU_META[level as KhushuLevel] ?? undefined;
}

export interface PrayerLog {
  date: string; // YYYY-MM-DD
  prayers: Partial<Record<PrayerName, PrayerStatus>>;
  // حضورُ القلب لكلّ فرضٍ أُدِّي — سؤالٌ واحدٌ يلي التسجيل مباشرةً، ويُتخطّى
  // بلا أثر. مخزَّنٌ على مفتاح اليوم نفسِه (لا مجموعةٍ ثالثة) فيُقرأ مع الحالة
  // بمرورٍ واحد، وبطابعٍ مستقلٍّ لكلّ صلاة: الإجابةُ تأتي بعد التسجيل بلحظة أو
  // بساعة، فلا يجوز أن تُحرّك طابعَ الحالة نفسِها ولا أن يمسحها تصحيحُ الحالة
  // على جهازٍ آخر.
  khushu?: Partial<Record<PrayerName, KhushuLevel>>;
  khushuUpdatedAt?: Partial<Record<PrayerName, number>>;
...
export interface DailyBudget {
  ...
  carryAdjust?: number;
  // **حصّة اليوم من تمويل المظاريف المموَّل من الراتب في هذه الدورة**:
  // مجموع ما يُحوَّل من راتب الدورة إلى المظاريف ÷ طول الدورة. يُطرح من البدل
  // فيصير «البدل الفعليّ» (`DailyBudgetStatus.rate`). ولماذا قطرةً يومية لا
  // خصماً دفعةً واحدة؟ لأنّ خصم ٥٠٠ يوم نزول الراتب يجعل رصيدك سالباً من اليوم
  // الأوّل بلا سبب؛ القطرة تنقص البدل بالتساوي وتقول لك مقدارها سلفاً — وهذا
  // كلّ المكسب: تعرفه مقدّماً بدل أن ينفجر في يوم. يُحسب في `confirmSalary`.
...
// الاسمان محجوزان لدلالتين مختلفتين: «عام» مظروفٌ عامّ يعبّر عن احتياطي
// المالك، و«الفوائض» وعاءٌ تقنيّ تستعمله الميزانية اليومية للمقاصة والترحيل.
// لا تخلط بينهما عند اختيار سلوكٍ خاصّ بالمظروف.
export const GENERAL_FUND_NAME = "عام";

// اسم صندوق الفوائض الذي يستقبل باقي الميزانية اليومية عند نزول الراتب.
export const SURPLUS_FUND_NAME = "الفوائض";

/** **قيدُ المطابقة الربعية.** كلَّ ثلاثة أشهر يفتح المالك كشوفَ حساباته ويكتب
 *  رقماً واحداً؛ يُسجَّل الفرقُ بين ما ظنّه التطبيق وما في الحساب فعلاً تسويةً
 *  على «الفوائض». القاعدةُ والحسابُ في `reconcile.ts` — لا معادلةَ هنا.
 *
 *  والقيدُ يُحفظ ولا يُمحى: تاريخُ التسويات هو ما يُظهر للمالك أنّ فرقَه يضيق
 *  دورةً بعد دورة (أو أنّه يتّسع، فيبحث عن سببه). */
export interface Reconcile {
  id: string;
  date: string; // YYYY-MM-DD
  /** ما ظنّه التطبيق لحظتَها (مظاريفُك + رصيدُ دورتك). */
  expected: number;
  /** ما أدخلتَه من كشوفك. */
  actual: number;
...
export interface AppData {
  ...
  // **المقاصة التلقائية**: حين ينزل رصيد الميزانية اليومية تحت الصفر يُسحب
  // العجز من صندوق «الفوائض» فوراً بلا ضغطة — بسقفٍ واحد: عجزٌ يتجاوز ثلاث
  // يوميّات ليس عجزاً يومياً بل حدثٌ سُجّل في المكان الخطأ، فيقف الحساب ويسأل
  // (القاعدة والسقف في `budgetFlow.ts`). غيابه في بياناتٍ قديمة = مفعّلة.
  autoOffset?: boolean;
  // **المطابقةُ الربعية**: سجلُّ التسويات بين ما يظنّه التطبيق وما في الحساب
  // فعلاً (`reconcile.ts`). اختياريّ فبياناتٌ قديمةٌ بلا الحقل تعمل.
