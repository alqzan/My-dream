...
export interface ReserveFund {
  ...
  funding?: FundFunding;
  // **وضع السفر**: حين يبدأ المالك رحلةً على هذا المظروف، يصير كلُّ مصروفٍ
  // يسجّله محسوباً عليها تلقائياً حتى ينهيها — فلا يُسأل عن الوجهة عند كلّ
  // فاتورةٍ وهو في الطريق. والتاريخان يجعلان السؤال الأخير ممكناً: «كم كلّفتني
  // رحلةُ المدينة، وفي كم يوم؟». الجاريةُ هي التي لا `endedAt` لها، وواحدةٌ في
  // كلّ وقتٍ عبر المظاريف كلِّها (`startTrip` تُنهي ما قبلها).
  //
