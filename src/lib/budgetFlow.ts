...
/** معرّفُ إيداعِ المقاصة التلقائية ليومٍ واحد على مظروفٍ واحد — **مشتقٌّ لا
 *  عشوائيّ**، وهذا هو ما يجعلها محايدةَ التكرار عبر الأجهزة.
 *
 *  المقاصةُ يُطلقها مراقبٌ يعمل على **كلّ** جهازٍ مفتوح بعد أيّ تغيّرٍ في
 *  المعاملات — ودمجُ لقطة السحابة من ذلك التغيّر. فجهازان مفتوحان يريان
 *  المعاملةَ المسبِّبة للعجز نفسَها ويسحبان كلاهما. وبمعرّفٍ عشوائيّ ينجو
 *  السحبان معاً في الدمج (الإيداعات تتّحد بالمعرّف) بينما `carryAdjust` مفردٌ
 *  يُحسم مرّةً واحدة: **«الفوائض» تُخصم مرّتين والميزانية تُرصَد مرّة**، ولا
 *  مسارَ في التطبيق يُصالح الفرق. وبمعرّفٍ مشتقٍّ ينهار السحبان إلى واحد. */
export function offsetDepositId(fundId: string, date: string): string {
  return `offset:${fundId}:${date}`;
}

/** هل هذا إيداعُ مقاصةٍ تلقائية؟ الدمجُ يحتاجها ليحسم تعارضَها بالأكبر. */
export function isOffsetDepositId(id: string): boolean {
