  ...
  return (
    <div className="mdr-three-arcs" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, margin: "14px 0 0" }}>
      {arcs.map((a) => (
        ...
        >
          <span className="mdr-three-arc-visual">
            <svg className="mdr-three-arc-art" viewBox="0 0 100 132" aria-hidden>
              <defs>
            ...
            <span className="mdr-three-arc-value" aria-hidden="true">
              <span style={{ fontSize: bigFitSize(a.big), color: a.color }}>{a.big}</span>
              <small>{a.unit}</small>
            </span>
          </span>

          <span className="mdr-three-arc-label">{a.label}</span>
          <span className="mdr-three-arc-sub">{a.sub}</span>
        </button>
      ))}
    </div>
  );
