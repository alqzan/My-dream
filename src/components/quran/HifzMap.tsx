...
const MAP_UNITS: { key: MapUnit; label: string; word: string }[] = [
  { key: "juz", label: "أجزاء", word: "الجزء" },
...
export function HifzMap({ text, onReview, onRead }: { text: string[] | null; onReview: (p: Portion) => void; onRead: (surah: number) => void }) {
  ...
  return (
    <div className="rounded-2xl border border-quran/20 bg-gradient-to-b from-quran/[0.06] to-transparent p-4 space-y-3.5">
      ...
      {/* مبدّل الحبيبة */}
      <div className="flex gap-1.5 p-0.5 rounded-xl bg-gray-100 dark:bg-[#2c2318]">
        {MAP_UNITS.map((u) => (
          <button key={u.key} onClick={() => { setUnit(u.key); setSel(null); }}
            className={`flex-1 text-xs font-semibold rounded-lg py-1.5 press transition-colors ${unit === u.key ? "bg-quran text-white shadow-sm" : "text-gray-500"}`}>
            {u.label}
          </button>
        ))}
      </div>
    ...
    <div className="rounded-xl border border-quran/20 bg-white dark:bg-[#241c12] p-3.5 animate-fade-up">
      ...
      <div className="flex gap-2 mt-2.5">
        {cell.state !== "none" && cell.memEnd >= cell.memStart && (
          <button onClick={() => onReview({ fromId: cell.memStart, toId: cell.memEnd })}
            className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg bg-quran/10 hover:bg-quran/20 text-quran text-xs font-semibold press">
