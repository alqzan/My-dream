...
export function HikmaCard() {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-brand-200 bg-gradient-to-l from-brand-50 to-white p-4">
      {/* Faded khatim star ornament in the corner */}
      <svg
        viewBox="0 0 60 60" aria-hidden
        className="absolute -left-4 -bottom-4 w-24 h-24 opacity-[0.08] text-brand-700"
      >
      ...
      <div className="relative">
        <div className="text-[11px] font-semibold text-brand-600 mb-1.5">✨ زاد اليوم</div>
        <p className="text-[15px] font-bold text-gray-800 leading-relaxed">{hikma.text}</p>
        <p className="text-[11px] text-gray-400 mt-1.5">— {hikma.source}</p>
      </div>
    </div>
  );
}
